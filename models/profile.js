...
module.exports.createProfile = async (profileData) => {
  ...
  try {
    // Log the profile data before insertion
    console.log("Profile data to insert:", profileData);

    // Convert 'null' string to actual null for experienceYears
    ...
    // Validate numeric fields (experienceYears, poste_id, grade_id, metier_id)
    if (experienceYears !== null && isNaN(experienceYears)) {
      throw new Error("Invalid experienceYears value");
    }
    if (poste_id !== null && isNaN(poste_id)) {
      throw new Error("Invalid poste_id value");
    }
    if (grade_id !== null && isNaN(grade_id)) {
      throw new Error("Invalid grade_id value");
    }
    if (metier_id !== null && isNaN(metier_id)) {
      throw new Error("Invalid metier_id value");
    }

    // Prepare the data with converted values
    const dataToInsert = {
      ...profileData,
      experienceYears,
      poste_id,
